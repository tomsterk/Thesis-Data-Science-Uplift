WITH
  ...
  covariates_churn AS (
    SELECT
      ...
      COUNT(
        DISTINCT CASE
          WHEN fbi.date_trading_nk >= add_months('2026-02-12', -24) THEN fbi.basket_sk
        END
      ) AS num_orders_52wk,
      SUM(
        CASE
          WHEN fbi.date_trading_nk >= add_months('2026-02-12', -24) THEN fbi.price_total_excl_vat
          ELSE 0
        END
      ) AS sales_52wk,
      SUM(
        CASE
          WHEN fbi.date_trading_nk >= add_months('2026-02-12', -24) THEN fbi.quantity
          ELSE 0
        END
      ) AS volume_52wk,
      SUM(
        CASE
          WHEN fbi.date_trading_nk >= add_months('2026-02-12', -24)
      ...
      COUNT(
        DISTINCT CASE
          WHEN fbi.date_trading_nk >= add_months('2026-02-12', -36)
          AND fbi.date_trading_nk < add_months('2026-02-12', -24) THEN fbi.basket_sk
        END
      ) AS num_orders_53w_104w,
      SUM(
        CASE
          WHEN fbi.date_trading_nk >= add_months('2026-02-12', -36)
          AND fbi.date_trading_nk < add_months('2026-02-12', -24) THEN fbi.price_total_excl_vat
          ELSE 0
        END
      ) AS sales_53w_104w,
      SUM(
        CASE
          WHEN fbi.date_trading_nk >= add_months('2026-02-12', -36)
          AND fbi.date_trading_nk < add_months('2026-02-12', -24) THEN fbi.quantity
          ELSE 0
        END
      ) AS volume_53w_104w,
      SUM(
        CASE
          WHEN fbi.date_trading_nk >= add_months('2026-02-12', -36)
          AND fbi.date_trading_nk < add_months('2026-02-12', -24)
          AND sales_channel_l1_name LIKE 'Online' THEN fbi.price_total_excl_vat
          ELSE 0
        END
...
select 
treatment_indicator,
sum(cameback) reactivated,
count(distinct customer_nk),
TO_CHAR(
    (
        SUM(cameback)::NUMERIC
        /
        NULLIF(COUNT(DISTINCT customer_nk), 0)::NUMERIC
    ) * 100,
    'FM999999990.000'
) || '%' AS comeback_rate,
  /* --- Reactivated-only averages --- */
  AVG(CASE WHEN cameback = 1 THEN recency END) AS avg_recency_reactivated,
  AVG(CASE WHEN cameback = 1 THEN monetary_value END) AS avg_monetary_value_reactivated,
  AVG(CASE WHEN cameback = 1 THEN length_of_relationship END) AS avg_length_of_relationship_reactivated,
  AVG(CASE WHEN cameback = 1 THEN online_sales END) AS avg_online_sales_reactivated,

  AVG(CASE WHEN cameback = 1 THEN retail_sales END) AS avg_retail_sales_reactivated
...
from (SELECT
  ...
  ('2026-02-12' - c.first_transaction_date) AS length_of_relationship,
  c.online_sales,
  c.retail_sales,
  c.food_total,
  c.vhms_total,
...
FROM
  ...
  LEFT JOIN customers_campaign ec ON c.customer_nk = ec.customer_nk)
  group by 1
  order by 4 desc